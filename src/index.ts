...
const generateTasks = async (
  ...
  for (let i = 0; i < count; i++) {
    const { ROOM_ID: roomId, USER_ID: userId } =
      faker.helpers.arrayElement(userRoomPairs);
    const { ID: groupId } = faker.helpers.arrayElement(
...
const generateComments = async (
  ...
  for (let i = 0; i < count; i++) {
    const { ROOM_ID: roomId, USER_ID: userId } =
      faker.helpers.arrayElement(userRoomPairs);
    const { ID: taskId } = faker.helpers.arrayElement(
    ...
    const request = connection.query(
      `CALL INSERT_TASK(${params.taskId},  ${params.userId}, ${params.content});`
    );
